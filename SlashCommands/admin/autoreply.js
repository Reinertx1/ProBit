...
module.exports = {
  ...
  run:async(client, interaction,args,guildData) => {
    ...
    if(subCommand === "add"){
      ...
      if(trigger.length > 300 || response.length > 300) return interaction.reply(`**🙄 - I can't add more than 300 characters for the trigger or the response .**`)
      let obj = {
    ...
    if(subCommand === "remove"){
      let target = interaction.options.getNumber("target")
